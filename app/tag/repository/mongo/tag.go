...
import (
	...
	"github.com/coocn-cn/leanote/app/tag/repository"
...
func (m *tag) Save(ctx context.Context, models ...*model.Tag) error {
	for _, model := range models {
		...
		if mut.db() != nil {
			// 更新之
			if ok := db.Upsert(m.collection, bson.M{"_id": saveData.UserId}, bson.M{"$set": mut.updates}); !ok {
				return errcode.NotFound(ctx, "更新对象失败", mut.db, saveData)
			}
		} else {
			// 保存之
			db.Insert(m.collection, info.Tag(saveData))
		}

		// 更新 model.mutation 对象状态
		*mut = *newtagMutation(saveData, (*info.Tag)(&saveData))
...
func (m *tag) Delete(ctx context.Context, models ...*model.Tag) error {
	for _, model := range models {
		...
		if ok := db.Delete(m.collection, bson.M{"_id": data.UserId}); !ok {
